...
class DetrDecoderOutput(BaseModelOutputWithCrossAttentions):
    ...
    Args:
        ...
        cross_attentions (`tuple(Tensor)`, *optional*, returned when `output_attentions=True` and `config.add_cross_attention=True` is passed or when `config.output_attentions=True`):
            Tuple of `Tensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
            sequence_length)`. Attentions weights of the decoder's cross-attention layer, after the attention softmax,
            used to compute the weighted average in the cross-attention heads.
        intermediate_hidden_states (`Tensor` of shape `(config.decoder_layers, batch_size, num_queries, hidden_size)`, *optional*, returned when `config.auxiliary_loss=True`):
...
class DetrModelOutput(Seq2SeqModelOutput):
    ...
    Args:
        ...
        encoder_attentions (`tuple(Tensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            weighted average in the self-attention heads.
        intermediate_hidden_states (`Tensor` of shape `(config.decoder_layers, batch_size, sequence_length, hidden_size)`, *optional*, returned when `config.auxiliary_loss=True`):
            Intermediate decoder activations, i.e. the output of each decoder layer, each of them gone through a
            layernorm.
    """
...
class DetrObjectDetectionOutput(ModelOutput):
    ...
    Args:
        loss (`Tensor` of shape `(1,)`, *optional*, returned when `labels` are provided)):
            Total loss as a linear combination of a negative log-likehood (cross-entropy) for class prediction and a
            bounding box loss. The latter is defined as a linear combination of the L1 loss and the generalized
            scale-invariant IoU loss.
        loss_dict (`Dict`, *optional*):
            A dictionary containing the individual losses. Useful for logging.
        logits (`Tensor` of shape `(batch_size, num_queries, num_classes + 1)`):
        ...
        last_hidden_state (`Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
            Sequence of hidden-states at the output of the last layer of the decoder of the model.
        decoder_hidden_states (`tuple(Tensor)`, *optional*, returned when `output_hidden_states=True` is passed or when `config.output_hidden_states=True`):
            Tuple of `Tensor` (one for the output of the embeddings + one for the output of each layer) of
            shape `(batch_size, sequence_length, hidden_size)`. Hidden-states of the decoder at the output of each
...
class DetrSegmentationOutput(ModelOutput):
    ...
    Args:
        ...
        pred_boxes (`Tensor` of shape `(batch_size, num_queries, 4)`):
            ...
            possible padding). You can use [`~DetrImageProcessor.post_process_object_detection`] to retrieve the
            unnormalized bounding boxes.
        pred_masks (`Tensor` of shape `(batch_size, num_queries, height/4, width/4)`):
...
class DetrConvEncoder(nn.Cell):
    ...
    def construct(self, pixel_values: Tensor, pixel_mask: Tensor):
        ...
        for feature_map in features:
            ...
            mask = ops.interpolate(pixel_mask[None].float(), size=feature_map.shape[-2:]).to(ms.bool_)[0]
...
class DetrSinePositionEmbedding(nn.Cell):
    ...
    def construct(self, pixel_values, pixel_mask):
        ...
        dim_t = ops.arange(self.embedding_dim).float()
...
class DetrAttention(nn.Cell):
    ...
    def with_pos_embed(self, tensor: Tensor, object_queries: Optional[Tensor]):
    ...
    def construct(
        ...
        object_queries: Optional[Tensor] = None,
        key_value_states: Optional[Tensor] = None,
        spatial_position_embeddings: Optional[Tensor] = None,
    ...
    def construct(
        ...
        Args:
            ...
            attention_mask (`Tensor`): attention mask of size
                ...
                values.
            object_queries (`Tensor`, *optional*):
...
class DetrDecoderLayer(nn.Cell):
    ...
    def construct(
        ...
        attention_mask: Optional[Tensor] = None,
        object_queries: Optional[Tensor] = None,
        query_position_embeddings: Optional[Tensor] = None,
        ...
        Args:
            ...
            query_position_embeddings (`Tensor`, *optional*):
...
class DetrEncoder(DetrPreTrainedModel):
    ...
    def construct(
        ...
        Args:
            ...
            object_queries (`Tensor` of shape `(batch_size, sequence_length, hidden_size)`):
...
class DetrDecoder(DetrPreTrainedModel):
    ...
    def construct(
        ...
        Args:
            ...
            object_queries (`Tensor` of shape `(batch_size, sequence_length, hidden_size)`, *optional*):
                Object queries that are added to the queries and keys in each cross-attention layer.
            query_position_embeddings (`Tensor` of shape `(batch_size, num_queries, hidden_size)`):
...
class DetrModel(DetrPreTrainedModel):
    ...
    ) -> Union[Tuple[Tensor], DetrModelOutput]:
        r"""
        Returns:
        Examples:
        ```python
...
class DetrForObjectDetection(DetrPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[Tensor], DetrObjectDetectionOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
            Labels for computing the bipartite matching loss. List of dicts, each dictionary containing at least the
            following 2 keys: 'class_labels' and 'boxes' (the class labels and bounding boxes of an image in the batch
            respectively). The class labels themselves should be a `Tensor` of len `(number of bounding boxes
            in the image,)` and the boxes a `Tensor` of shape `(number of bounding boxes in the image, 4)`.
        Returns:
        Examples:
        ```python
...
class DetrForSegmentation(DetrPreTrainedModel):
    ...
    def construct(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[Tensor], DetrSegmentationOutput]:
        r"""
        labels (`List[Dict]` of len `(batch_size,)`, *optional*):
            Labels for computing the bipartite matching loss, DICE/F-1 loss and Focal loss. List of dicts, each
            dictionary containing at least the following 3 keys: 'class_labels', 'boxes' and 'masks' (the class labels,
            bounding boxes and segmentation masks of an image in the batch respectively). The class labels themselves
            should be a `Tensor` of len `(number of bounding boxes in the image,)`, the boxes a
            `Tensor` of shape `(number of bounding boxes in the image, 4)` and the masks a
            `Tensor` of shape `(number of bounding boxes in the image, height, width)`.
        Returns:
        Examples:
        ```python
        >>> import io
...
def sigmoid_focal_loss(inputs, targets, num_boxes, alpha: float = 0.25, gamma: float = 2):
    ...
    Args:
        inputs (`Tensor` of arbitrary shape):
            The predictions for each example.
        targets (`Tensor` with the same shape as `inputs`)
...
class DetrHungarianMatcher(nn.Cell):
    ...
    def construct(self, outputs, targets):
        ...
        return [(Tensor(i, dtype=ms.int64), Tensor(j, dtype=ms.int64)) for i, j in indices]
...
def box_area(boxes: Tensor) -> Tensor:
    ...
    Args:
        boxes (`Tensor` of shape `(number_of_boxes, 4)`):
            Boxes for which the area will be computed. They are expected to be in (x1, y1, x2, y2) format with `0 <= x1
            < x2` and `0 <= y1 < y2`.
    Returns:
        `Tensor`: a tensor containing the area for each box.
    """
    boxes = _upcast(boxes)
...
def generalized_box_iou(boxes1, boxes2):
    ...
    Returns:
        `Tensor`: a [N, M] pairwise matrix, where N = len(boxes1) and M = len(boxes2)
    """
