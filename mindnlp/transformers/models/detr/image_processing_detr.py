...
def convert_segmentation_to_rle(segmentation):
    ...
    Args:
        segmentation (`Tensor` or `numpy.array`):
...
def remove_low_and_no_objects(masks, scores, labels, object_mask_threshold, num_labels):
    ...
    Returns:
        `Tuple[`Tensor`, `Tensor`, `Tensor`]`: The `masks`, `scores` and `labels` without the region
...
class DetrImageProcessor(BaseImageProcessor):
        ...
        Args:
            ...
            return_tensors (`str` or `TensorType`, *optional*):
                The type of tensors to return. Can be one of:
                    ...
                    - `TensorType.PYTORCH` or `'pt'`: Return a batch of type `Tensor`.
        ...
        **kwargs,
        ...
        if not valid_images(images):
            raise ValueError(
                ...
                "Tensor, tf.Tensor or jax.ndarray."
            )
    ...
    def post_process_segmentation(self, outputs, target_sizes, threshold=0.9, mask_threshold=0.5):
        ...
        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            target_sizes (`Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
    ...
    def post_process_instance(self, results, outputs, orig_target_sizes, max_target_sizes, threshold=0.5):
        ...
        Args:
            ...
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            orig_target_sizes (`Tensor` of shape `(batch_size, 2)`):
                Tensor containing the size (h, w) of each image of the batch. For evaluation, this must be the original
                image size (before any data augmentation).
            max_target_sizes (`Tensor` of shape `(batch_size, 2)`):
    ...
    def post_process_panoptic(self, outputs, processed_sizes, target_sizes=None, is_thing_map=None, threshold=0.85):
        ...
        Args:
            outputs ([`DetrSegmentationOutput`]):
                Raw outputs of the model.
            processed_sizes (`Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`):
                Torch Tensor (or list) containing the size (h, w) of each image of the batch, i.e. the size after data
                augmentation but before batching.
            target_sizes (`Tensor` of shape `(batch_size, 2)` or `List[Tuple]` of length `batch_size`, *optional*):
            ...
            is_thing_map (`Tensor` of shape `(batch_size, 2)`, *optional*):
        ...
        for cur_logits, cur_masks, cur_boxes, size, target_size in zip(
            ...
            def get_ids_area(masks, scores, dedup=False):
                ...
                m_id = masks.swapaxes(0, 1).softmax(-1)
    ...
    def post_process_object_detection(
        ...
        Args:
            ...
            target_sizes (`Tensor` or `List[Tuple[int, int]]`, *optional*):
        ...
        if target_sizes is not None:
            if isinstance(target_sizes, List):
                img_h = Tensor([i[0] for i in target_sizes])
                img_w = Tensor([i[1] for i in target_sizes])
            else:
                img_h, img_w = target_sizes.unbind(1)
    ...
    def post_process_semantic_segmentation(self, outputs, target_sizes: List[Tuple[int, int]] = None):
        ...
        Returns:
            `List[Tensor]`:
                ...
                `Tensor` correspond to a semantic class id.
        """
