    ...
    def prepare_config_and_inputs(self):
        ...
        if self.use_labels:
            ...
            for i in range(self.batch_size):
                target = {}
                target["class_labels"] = ops.randint(low=0, high=self.num_labels, size=(self.n_targets,))
...
class DetrModelIntegrationTestsTimmBackbone(unittest.TestCase):
    ...
    def test_inference_object_detection_head(self):
        ...
        # verify outputs
        expected_shape_logits = ops.ones((1, model.config.num_queries, model.config.num_labels + 1))
        ...
        expected_shape_boxes = ops.ones((1, model.config.num_queries, 4)).shape
        ...
        expected_scores = ms.Tensor([0.9982, 0.9960, 0.9955, 0.9988, 0.9987])
        expected_labels = [75, 75, 63, 17, 17]
        expected_slice_boxes = ms.Tensor([40.1633, 70.8115, 175.5471, 117.9841])
    ...
    def test_inference_panoptic_segmentation_head(self):
        ...
        # verify outputs
        expected_shape_logits = ops.ones((1, model.config.num_queries, model.config.num_labels + 1)).shape
        self.assertEqual(outputs.logits.shape, expected_shape_logits)
        expected_slice_logits = ms.Tensor(
        ...
        expected_shape_masks = ops.ones((1, model.config.num_queries, 200, 267)).shape
        self.assertEqual(outputs.pred_masks.shape, expected_shape_masks)
        expected_slice_masks = ms.Tensor(
            [[-7.7558, -10.8788, -11.9797], [-11.8881, -16.4329, -17.7451], [-14.7316, -19.7383, -20.3004]]
        )
        self.assertTrue(np.allclose(outputs.pred_masks[0, 0, :3, :3].asnumpy(), expected_slice_masks.asnumpy(), atol=1e-3))
        ...
        expected_shape = ops.ones([480, 640])
        expected_slice_segmentation = ms.Tensor([[4, 4, 4], [4, 4, 4], [4, 4, 4]], dtype=ms.int32)
        ...
        self.assertTrue(np.allclose(results["segmentation"][:3, :3].asnumpy(), expected_slice_segmentation.asnumpy(), atol=1e-4))
