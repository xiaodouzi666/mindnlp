...
class DetrImageProcessingTest(AnnotationFormatTestMixin, ImageProcessingTestMixin, unittest.TestCase):
    ...
    def test_call_pytorch_with_coco_detection_annotations(self):
        ...
        # verify pixel values
        expected_shape = ops.zeros([1, 3, 800, 1066])
        self.assertEqual(encoding["pixel_values"].shape, expected_shape.shape)
    ...
    def test_call_pytorch_with_coco_panoptic_annotations(self):
        ...
        # verify pixel values
        expected_shape = ops.ones([1, 3, 800, 1066]).shape
        ...
        # verify area
        expected_area = ms.Tensor([147979.6875, 165527.0469, 484638.5938, 11292.9375, 5879.6562, 7634.1147])
        self.assertTrue(np.allclose(encoding["labels"][0]["area"].asnumpy(), expected_area.asnumpy()))
        # verify boxes
        expected_boxes_shape = ops.ones([6, 4]).shape
        self.assertEqual(encoding["labels"][0]["boxes"].shape, expected_boxes_shape)
    ...
    def test_batched_coco_detection_annotations(self):
        ...
        expected_shape = ops.ones([2, 3, postprocessed_height, postprocessed_width]).shape
        self.assertEqual(encoding["pixel_values"].shape, expected_shape)

        # Check the bounding boxes have been adjusted for padded images
        self.assertEqual(encoding["labels"][0]["boxes"].shape, ops.ones([6, 4]))
        self.assertEqual(encoding["labels"][1]["boxes"].shape, ops.ones([6, 4]))
        ...
        self.assertTrue(np.allclose(encoding["labels"][0]["boxes"].asnumpy(), expected_boxes_0.asnumpy(), rtol=1e-3))
        self.assertTrue(np.allclose(encoding["labels"][1]["boxes"].asnumpy(), expected_boxes_1.asnumpy(), rtol=1e-3))

        # Check the masks have also been padded
        self.assertEqual(encoding["labels"][0]["masks"].shape, ops.ones([6, 800, 1066]).shape)
        self.assertEqual(encoding["labels"][1]["masks"].shape, ops.ones([6, 800, 1066]).shape)
        ...
        self.assertEqual(encoding["labels"][0]["boxes"].shape, ops.ones([6, 4]).shape)
        self.assertEqual(encoding["labels"][1]["boxes"].shape, ops.ones([6, 4]).shape)
    ...
    def test_max_width_max_height_resizing_and_pad_strategy(self):
        ...
        # do_pad=False, max_height=100, width=100, image=200x100 -> 100x50
        image_processor = DetrImageProcessor(
            size={"height": 100, "width": 50},
            do_pad=False,
        )
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 100, 50]).shape)

        # do_pad=False, height=300, width=100, image=200x100 -> 200x100
        image_processor = DetrImageProcessor(
        ...
        # do_pad=True, height=100, width=100, image=200x100 -> 100x100
        image_processor = DetrImageProcessor(
            size={"height": 100, "width": 100}, do_pad=True, pad_size={"height": 100, "width": 100}
        )
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 100, 100]).shape)

        # do_pad=True, height=300, width=100, image=200x100 -> 300x100
        image_processor = DetrImageProcessor(
        ...
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 301, 101]).shape)

        ### Check for batch
        image_2 = ops.ones([100, 150, 3], dtype=ms.uint8)

        # do_pad=True, height=150, width=100, images=[200x100, 100x150] -> 150x100
        image_processor = DetrImageProcessor(
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([2, 3, 150, 100]).shape)

    def test_longest_edge_shortest_edge_resizing_strategy(self):
        image_1 = ops.ones([958, 653, 3], dtype=ms.uint8)
        ...
        inputs = image_processor(images=[image_1], return_tensors="ms")
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 640, 436]).shape)

        image_2 = ops.ones([653, 958, 3], dtype=ms.uint8)
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 436, 640]).shape)

        image_3 = ops.ones([100, 120, 3], dtype=ms.uint8)
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 98, 118]).shape)

        image_4 = ops.ones([128, 50, 3], dtype=ms.uint8)
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 128, 50]).shape)

        image_5 = ops.ones([50, 50, 3], dtype=ms.uint8)
        ...
        self.assertEqual(inputs["pixel_values"].shape, ops.ones([1, 3, 50, 50]).shape)